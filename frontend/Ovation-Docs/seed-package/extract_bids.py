...
# Real L2 baseline per trade for TradePackages.EstimatedCostL2 (variance feature).
# We classify L2 lines with the SAME rules but DO NOT persist group_key on them
# (group_key is L3-only per schema) -- this is a read-only roll-up.
L2_LEVEL_ID = "west-henderson-l2-1"
l2_total_by_trade = {}
for x in line_items:
    if x["budget_level_id"] != L2_LEVEL_ID:
        continue
    s = classify(x)
    if s:
        l2_total_by_trade[s] = l2_total_by_trade.get(s, 0.0) + x["line_total"]
...
for sort_order, (slug, title, match, benchmark, real) in enumerate(TRADES):
    ...
    trade_packages.append({
        ...
        "trade_key": slug,
        "title": title,
        "estimated_cost_l2": r2(l2_total_by_trade.get(slug, 0.0)),   # real L2 roll-up
        "status": "Awarded",
